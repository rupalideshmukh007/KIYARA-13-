...
export type KiyaraData = {
  brain: Record<string, BrainData>;
  knowledge: Record<string, KnowledgeData>;
};

const initialKiyaraData: KiyaraData = {
  brain: {
    greetings: {
...
let kiyaraData: KiyaraData = initialKiyaraData;

export async function loadKiyaraData() {
  try {
    const storedData = await AsyncStorage.getItem("kiyara-data");
    if (storedData) {
      kiyaraData = JSON.parse(storedData);
    } else {
      await saveKiyaraData();
    }
  } catch (e) {
    console.error("Failed to load Kiyara data.", e);
  }
}

export async function saveKiyaraData() {
  try {
    await AsyncStorage.setItem("kiyara-data", JSON.stringify(kiyaraData));
  } catch (e) {
    console.error("Failed to save Kiyara data.", e);
  }
}

export function getKiyaraData() {
  return kiyaraData;
}

export function updateKiyaraData(newData: KiyaraData) {
  kiyaraData = newData;
  saveKiyaraData();
}
...
export function getKiyaraReply(input: string): string {
  loadKiyaraData(); // Ensure data is loaded
  const text = input.toLowerCase();
  const lang = detectLang(input);

  // Try to find a reply from the knowledge base first
  for (const [, know] of Object.entries(kiyaraData.knowledge)) {
  ...
  for (const [, data] of Object.entries(kiyaraData.brain)) {
